...
{
   [Export("ThrustDetector", typeof(IPatternDetector))]
   [PartCreationPolicy(CreationPolicy.NonShared)]
   public class ThrustDetector00 : ThrustDetectorBase
   {
      short _minSequentialPositive3x3Count;
      short _maxSequentialNegative3x3Count;
      ...
      public override void Initialize(IParameters parameters)
      {
         ...
         _minSequentialPositive3x3Count = _parameters.GetShort("minsequentialpositive3x3count") ?? 6;
         _maxSequentialNegative3x3Count = _parameters.GetShort("maxsequentialnegative3x3count") ?? 3;
      ...
      protected override Thrust DetectThrust()
      {
         ...
         if (_chart.Frames.Count >= 20)
         {
            ...
            while (loopCount < 20)
            {
               ...
               if (loopCount > 9)
               {
                  upThrustFound = sequentialAbove3x3Count() >= _minSequentialPositive3x3Count 
                     && sequentialBelow3x3Count() <= _maxSequentialNegative3x3Count
                     && CheckLeaders(leaderSides, AboveBelowDMA.Above);

                  downThrustFound = sequentialBelow3x3Count() >= _minSequentialPositive3x3Count 
                     && sequentialAbove3x3Count() <= _maxSequentialNegative3x3Count
