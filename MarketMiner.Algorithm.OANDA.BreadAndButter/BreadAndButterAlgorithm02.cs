...
using MarketMiner.Api.OANDA.TradeLibrary.DataTypes.Communications;
...
namespace MarketMiner.Algorithm.OANDA.Patterns
{
   ...
   [AlgorithmModule(Name = "BreadAndButterAlgorithm02", StrategyID = 1)]
   public class BreadAndButterAlgorithm02 : BreadAndButterAlgorithm01
   {
      [ImportingConstructor]
      public BreadAndButterAlgorithm02(IEventLogger logger)
         : base(logger)
      {
      }
      ...
      protected virtual void ProcessCharts()
      {
         ...
         {
            Thrust thrust = (Thrust)chart.Patterns.FirstOrDefault(p => p.Type == MACC.Constants.SignalType.Thrust && p.Active);

            if (thrust == null)
            {
               thrust = _thrustDetector.DetectPattern(chart) as Thrust;

               if (thrust != null && thrust.Active)
               {
      ...
      {
         if (thrust.StrategyTransactionID != null)
         {
      ...
      protected virtual async Task<int> CreateEntryOrder(MCE.Signal signal, Chart chart, Thrust thrust)
      {
         ...
         // catch bad orders here until I fix this
         //if (signal.Side == MACC.Constants.SignalSide.Buy)
         //{
         //   if (entryPrice > takeProfitPrice) tradeUnits = -96;
         //   if (entryPrice < stopLossPrice) tradeUnits = -97;
         //}
         //else
         //{
         //   if (entryPrice < takeProfitPrice) tradeUnits = -98;
         //   if (entryPrice > stopLossPrice) tradeUnits = -99;
         //}
         ...
         if (tradeUnits > 0)
         {
            ...
            if (response.orderOpened != null && response.orderOpened.id > 0)
            {
               ...
               thrust.SetTradePrices(entryPrice, takeProfitPrice, stopLossPrice, places);
            }
         }
         else
         {
      ...
      protected virtual async Task UpdateEntryOrder(StrategyTransaction orderTransaction, Chart chart, Thrust thrust)
      {
         ...
         if (tradeUnits > 0)
         {
            ...
            Order order = await Rest.PatchOrderAsync(_accountId, orderId, patchData);

            if (order != null)
            {
               MarketMinerOrder orderUpdated = new MarketMinerOrder(order);
               orderUpdated.SignalID = thrust.SignalID;
               orderUpdated.StrategyTransactionID = orderTransaction.StrategyTransactionID;

               orderTransaction.Price = orderUpdated.price;
               orderTransaction.Time = Convert.ToDateTime(orderUpdated.time).ToUniversalTime();
               orderTransaction = await StrategyCaller.Instance().UpdateStrategyTransactionAsync(orderTransaction);

               AddOrUpdateAlgorithmOrder(orderUpdated);
      ...
      protected virtual async Task UpdateOpenTrade(Thrust thrust, long tradeId, double? takeProfitPrice, double? stopLossPrice, int? places)
      {
         ...
         if (patchData.Count > 0)
         {
            ...
            thrust.SetTradePrices(null, takeProfitPrice, stopLossPrice, places);
         }
      }
      ...
      protected virtual async void ManageOrder(Chart chart, Thrust thrust)
      {
         ...
         else
         {
            if (exitTransaction == null)
            {
               ...
               Frame focusFrame = chart.Frames.FirstOrDefault(f => f.Bar.time == thrust.FocusTime);
               ...
               retracement.ExtremaPrice = SetExtremaPrice(chart, thrust.Side, focusIndex);
      ...
      /// <returns></returns>
      protected virtual double? SetExtremaPrice(Chart chart, string side, int focusIndex)
      {
         double? extremaPrice = null;

         // only if the chart has post-focus frames
         if (focusIndex + 1 < chart.Frames.Count)
         {
            // initialize
            if (side == MACC.Constants.SignalSide.Buy)
            ...
            // discover
            for (int i = focusIndex + 1; i < chart.Frames.Count; i++)
            {
               if (side == MACC.Constants.SignalSide.Buy)
