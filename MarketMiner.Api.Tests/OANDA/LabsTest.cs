...
{
   public class LabsTest : RestTestBase
   {
      #region Constructors
      public LabsTest(RestTestResults results)
         : base(results)
      {
      }
      #endregion
      ...
      {
         // usually fails due to no key levels (ie. null) returned
         var oandaAutoChartistTestData = new Dictionary<string, string> { { "type", "keylevel" } };

         // usually succeeds
         var marketMinerAutoChartistTestData = new Dictionary<string, string> { { "type", "chartpattern" } };

         var autochartistData = await Rest.GetAutochartistData(oandaAutoChartistTestData);

         if (autochartistData == null)
            autochartistData = await Rest.GetAutochartistData(marketMinerAutoChartistTestData);
      ...
      {
         var calendarEvents = await Rest.GetCalendarData("EUR_USD", 7776000);
