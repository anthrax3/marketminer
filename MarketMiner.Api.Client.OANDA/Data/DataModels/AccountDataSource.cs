...
namespace MarketMiner.Api.Client.OANDA.Data.DataModels
{
   ...
   public abstract class DataCommon : BindableBase
   {
      ...
      public Uri Image
      {
         get
         {
            ...
            {
               this._image = new Uri(DataCommon._baseUri, this._imagePath);
            }
            return this._image;
         }
      ...
      //      if (this._image == null && this._imagePath != null)
      //      {
      //         this._image = new BitmapImage(new Uri(MUIDataCommon._baseUri, this._imagePath));
      //      }
      //      return this._image;
      //   }
   ...
   public class DataGroup : DataCommon
   {
      ...
      protected readonly ObservableCollection<DataItem> _items = new ObservableCollection<DataItem>();
      public ObservableCollection<DataItem> Items
      {
         get { return this._items; }
      }

      protected readonly ObservableCollection<DataItem> _topItem = new ObservableCollection<DataItem>();
      public ObservableCollection<DataItem> TopItems
      {
         get { return this._topItem; }
      }

      public virtual void UpdateItems<T>(List<T> list)
      {
   ...
   public class HistoryDataGroup : DataGroup
   {
      ...
      public override void UpdateItems<T>(List<T> list)
      {
          ...
          Task.Run(async () =>
          {
              for (int t = 0; t < dataList.Count && t < this._maxEntries; t++)
              {
                  Items.Insert(t, dataList[t]);
                  if (Items.Count > this._maxEntries)
                  {
                      Items.RemoveAt(this._maxEntries);
                  }
              }
          }
          );
   ...
   public class AccountDataSource
   {
      ...
      public virtual void EnableUpdates()
      {
          ...
          {	// If we're not getting streamed the notifications, we'll have to poll
              if (_transTimer == null)
              {
                  _transTimer = new TaskTimer(Refresh, null, 0, 1000, true);
              }
          }
      }

      protected void CurrentSessionOnDataReceived(Event data)
      {
         InternalRefresh();
      }

      protected async Task InternalRefresh()
      {
      ...
      protected virtual async void Refresh(object state)
      {
      ...
      public virtual async void Refresh()
      {
          ...
          if (_transTimer != null)
          {
              if(!_transTimer.IsCancellationRequested) _transTimer.Cancel();
          }
          _transTimer = new TaskTimer(Refresh, null, 0, 1000, true);
      }
